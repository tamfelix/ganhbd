...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            .frame{ border: 5px solid rgb(242, 240, 240);
                box-shadow: 0 0 3px grey}
            .frame1{ border: 10px solid rgb(242, 240, 240);
                box-shadow: 0 0 3px grey}
        </style>
    </head>
    ...
    <section class="topmenu ">
        <ul class="flex inline-flex ">
            @foreach($topmenu as $item)
                <a href="{{ env('APP_URL').'/'.$item['link'] }}"><li class="list-none p-1">{{ $item['title_'.app()->getLocale()] }} |</li></a>
            @endforeach
        </ul>
    </section>
...
{{--INTERNATIONAL GAN--}}
            ...
            <a href="#" alt="international gan website">
            <div class="fixed top-0 right-32 px-6 py-1 sm:block">International gan</div>
            </a>


            @if (Route::has('login'))

                    <div class="hidden fixed top-8 right-0 px-6 py-1 sm:block">
                ...
                <x-header :title="$title" :link="$link" />
                    ...
                    <div class="flex inline-flex items-center px-4 m-auto">
                        @foreach($photos as $item)
                            <a href="#">
                                <img class="h-[200px] frame" src="{{ env('APP_URL').'/'.$item['link'] }}">
                            </a>
                        @endforeach
                    </div>
                </div>
...
{{--VISITS ENROLLMENT--}}
                @php $title = __('sections.register'); $link = route('emails.create'); @endphp
                <x-header :title="$title" :link="$link" />
                    <div class="flex h-full  py-4   ">
                    ..
                    </div>
...
{{--PARENTS--}}
                @php $title = __('sections.parents'); $link = url('toparents'); @endphp
                <x-header :title="$title" :link="$link" />
                    <div class="flex text-center m-auto h-full  py-4   px-12">
                        - <a>{{ __('sections.guideline') }}</a>
                    </div>
                </div>
                {{--                        FOOTER--}}
                ...
                <div class="flex justify-center mt-4 sm:items-center sm:justify-between">
                    ...
                    <div class="ml-4 text-center text-sm text-gray-500 sm:text-right sm:ml-0">
                        Gan Yossef Habad @ <?=date('Y') ?> (release v 1.0)
                    </div>
                </div>
            </div>
        </div>
